...
const SimpleAlbedo = () => {
  ...
  // Sort = fuld toner (≈5 %), hvid = upåtrykt mat papir (≈75 %). Et trykt
  // gråfelt kan ikke tildeles en troværdig albedo og bruges ikke længere.
  ...
  const handleStageMouseUp = () => {
    ...
    if (currentRect.width > 10 && currentRect.height > 10) {
      ...
      const isReference = imageType === 'photo' &&
        ((isReferenceMode && refCount < REFERENCE_PATCHES.length) ||
         (selections.length === 0 && refCount === 0));
      const newSelection = {
        id: Date.now(),
        ...currentRect,
        isReference,
      ...
      if (isReference) {
        // Reference card - add it and turn off reference mode
  ...
  const buildToAlbedo = () => {
    ...
    const refValues = refSelections.map((r) => getPixelValues(r).averageGray);
    if (refValues.length >= 2) {
      const v1 = refValues[0], v2 = refValues[refValues.length - 1];
      const a1 = REFERENCE_PATCHES[0].albedo;
      const a2 = REFERENCE_PATCHES[REFERENCE_PATCHES.length - 1].albedo;
      if (v1 > 0 && v2 > 0 && Math.abs(v2 - v1) > 1) {
        const b = Math.log(a2 / a1) / Math.log(v2 / v1);
        const a = a1 / Math.pow(v1, b);
        return { toAlbedo: (v) => (v > 0 ? Math.min(a * Math.pow(v, b), 100) : 0), mode: 'to referencefelter (potenslov)' };
      }
    }
    const vRef = refValues[refValues.length - 1];
    const aRef = REFERENCE_PATCHES[REFERENCE_PATCHES.length - 1].albedo;
    return { toAlbedo: (v) => (vRef > 0 ? Math.min((v / vRef) * aRef, 100) : 0), mode: 'ét referencefelt' };
  };
              ...
              {imageType === 'photo' ? (
                <ol style={{ marginLeft: "20px", fontSize: "0.9rem" }}>
                  <li style={{ marginBottom: "8px" }}>
                    Markér kortets <strong>sorte felt</strong> (albedo 5 %)
                  </li>
                  <li style={{ marginBottom: "8px" }}>
                    Markér det <strong>hvide papir</strong> (albedo 75 %)
                  </li>
                  <li style={{ marginBottom: "8px" }}>
              ...
              <div style={{ marginBottom: "20px", padding: "12px", backgroundColor: "#f0f9ff", borderRadius: "8px", border: "1px solid #bae6fd" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
                  <input
                    ...
                    onChange={(e) => setIsReferenceMode(e.target.checked)}
                    style={{ width: "18px", height: "18px", cursor: "pointer" }}
                  />
                  <span style={{ fontWeight: 600, color: isReferenceMode ? "#dc2626" : "#1e293b", fontSize: "0.9rem" }}>
                    Markér referencefelt (sort først, så hvidt)
                  </span>
                </label>
                {isReferenceMode && (
                  <p style={{ marginTop: "8px", marginLeft: "28px", color: "#dc2626", fontSize: "0.85rem" }}>
                    ✓ Reference-tilstand aktiveret
                  </p>
                )}
              </div>
            )}
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              {selections.length === 0 && (
                <p style={{ fontSize: "0.85rem", color: "#dc2626", margin: 0 }}>
                  ⚠ {imageType === 'photo' ? 'Start med kortets sorte felt' : 'Markér et område på billedet'}
                </p>
              )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                <div>
                  ...
                  <input
                    ...
                    style={{ width: "100%", padding: "6px", fontSize: "0.85rem", border: "1px solid #d1d5db", borderRadius: "4px" }}
                  />
                </div>
                <div>
                ...
                <div style={{ backgroundColor: "#ffffff", padding: "20px", borderRadius: "8px", border: "1px solid #e5e7eb" }}>
                  ...
                  {savedMeasurements.map((measurement, idx) => (
                    ...
                    >
                      ...
                      <div style={{ marginTop: "15px", padding: "12px", backgroundColor: "#f0f9ff", borderRadius: "4px", border: "1px solid #bae6fd" }}>
                        ...
                        {measurement.results.map((result, rIdx) => {
                          if (result.area.includes("referencekort") || result.area.includes("Reference")) {
                            return (
                              <div key={rIdx} style={{ marginTop: "10px", padding: "8px", backgroundColor: "white", borderRadius: "4px", fontSize: "0.85rem" }}>
